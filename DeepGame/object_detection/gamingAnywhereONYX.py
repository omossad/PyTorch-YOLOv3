...
sys.path.append('C:\\Users\\omossad\\Desktop\\codes\\Onnx\\pip')
import onnxruntime
...
if __name__ == "__main__":
    ...
    #onnx_model = onnxruntime.InferenceSession('fifa.onnx', sess_options)
    onnx_model = onnxruntime.InferenceSession('fifa.onnx')
    print("The device used by onnxruntime is :" +onnxruntime.get_device())
    ...
    while True:
        ...
        input_imgs = input_tensor.cpu().detach().numpy().astype(np.float32)
        od_start = time.time()
        ####
        time1 = time.time()
        detections = torch.from_numpy(np.squeeze(np.array(onnx_model.run(None,{onnx_model.get_inputs()[0].name: input_imgs })), axis=0))
        time11 = time.time()
