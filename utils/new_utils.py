...
def get_batch_statistic(outputs_x, outputs_y, targets):
    ...
    tx = torch.zeros([num_samples, num_tiles]).type(FloatTensor)
    ty = torch.zeros([num_samples, num_tiles]).type(FloatTensor)
    tx.scatter_(1, x_label, 1)
    ty.scatter_(1, y_label, 1)
    _, pred_x = torch.max(outputs_x, 1)
...
def non_max_suppression(prediction, conf_thres=0.3, nms_thres=0.2):
    """
