...
def evaluate(base_model, fine_model, path, conf_thres, nms_thres, img_size, num_tiles, classes, batch_size):
    ...
    for batch_i, (_, imgs, targets) in enumerate(tqdm.tqdm(dataloader, desc="Detecting objects")):
        ...
        with torch.no_grad():
            ...
            x_inpt1, x_inpt2, x_inpt3 = yolo_single_tile(yolo_outputs, num_tiles, classes, img_size)
            x_inpt1 = Variable(x_inpt1.type(Tensor), requires_grad=False)
            x_inpt2 = Variable(x_inpt2.type(Tensor), requires_grad=False)
            x_inpt3 = Variable(x_inpt3.type(Tensor), requires_grad=False)
            #y_inpt = Variable(y_inpt.to(device))
            #loss_h, output_x, h_score = fine_model_h(x_inpt, targets)
            loss, output, score = fine_model(x_inpt1, x_inpt2, x_inpt3, targets)
